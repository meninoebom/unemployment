<?php
	...
	if (mysqli_num_rows($query) > 0)
		while ($rd = $query->fetch_assoc()) {
			...
			echo "<feature id=\"" . strtolower($rd['countryID'] . "_" . $rd['stateID']) . "\" category=\"state_" . $category . "\" label=\"" . $rd['state'] . "\" label_map=\"" . strtoupper($rd['stateID']) . "\" popup=\"" . $y . " Population: " . number_format($rd['total'], 0) . "\" y=\"" . $y . "\" map=\"us\" />\n";
		}
	$query->close();
	$jqm_db->close();
